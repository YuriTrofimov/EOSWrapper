...
FEOSWrapperSessionManager::~FEOSWrapperSessionManager()
{
	...
	EOS_Lobby_RemoveNotifyLobbyUpdateReceived(EOSSubsystem->GetLobbyHandle(), LobbyUpdateReceivedId);
	EOS_Lobby_RemoveNotifyLobbyMemberUpdateReceived(EOSSubsystem->GetLobbyHandle(), LobbyMemberUpdateReceivedId);
	EOS_Lobby_RemoveNotifyLobbyMemberStatusReceived(EOSSubsystem->GetLobbyHandle(), LobbyMemberStatusReceivedId);
	EOS_Lobby_RemoveNotifyLobbyInviteAccepted(EOSSubsystem->GetLobbyHandle(), LobbyInviteAcceptedId);
	EOS_Lobby_RemoveNotifyJoinLobbyAccepted(EOSSubsystem->GetLobbyHandle(), JoinLobbyAcceptedId);
	...
	const FOnSingleSessionResultCompleteDelegate& CompletionDelegate)
{
	...
	EOS_EResult CreateLobbySearchResult = EOS_Lobby_CreateLobbySearch(EOSSubsystem->GetLobbyHandle(), &CreateLobbySearchOptions, &LobbySearchHandle);
...
void FEOSWrapperSessionManager::RemovePlayerFromSession(int32 LocalUserNum, FName SessionName, const FUniqueNetId& TargetPlayerId)
{
	...
	if (Session)
	{
		...
		EOS_Lobby_KickMember(EOSSubsystem->GetLobbyHandle(), &KickMemberOptions, CallbackObj, CallbackObj->GetCallbackPtr());
	}
	else
	{
...
bool FEOSWrapperSessionManager::SetLobbyParameter(const FName& LobbyName, const FName& Parameter, const FString& Value)
{
	...
	EOS_EResult LobbyModificationResult = EOS_Lobby_UpdateLobbyModification(EOSSubsystem->GetLobbyHandle(), &UpdateLobbyModificationOptions, &LobbyModificationHandle);
	if (LobbyModificationResult == EOS_EResult::EOS_Success)
	{
		...
		EOS_Lobby_UpdateLobby(EOSSubsystem->GetLobbyHandle(), &UpdateLobbyOptions, CallbackObj, CallbackObj->GetCallbackPtr());
...
void FEOSWrapperSessionManager::Initialize()
{
...
void FEOSWrapperSessionManager::Shutdown()
{
	EOS_ProductUserId ProductUserID = EOSSubsystem->UserManager->GetLocalProductUserId(0);

	// for (const auto& Lobby : LobbySessions)
	// {
	// 	EOS_Lobby_DestroyLobbyOptions Options = {};
	// 	Options.ApiVersion = EOS_LOBBY_DESTROYLOBBY_API_LATEST;
	// 	Options.LobbyId = Lobby.Value;
	// 	Options.LocalUserId = ProductUserID;
	// 	EOS_Lobby_DestroyLobby(EOSSubsystem->GetLobbyHandle(), &Options, nullptr, nullptr);
	// }
}
...
void FEOSWrapperSessionManager::RegisterLobbyNotifications()
{
	...
	LobbyUpdateReceivedId = EOS_Lobby_AddNotifyLobbyUpdateReceived(EOSSubsystem->GetLobbyHandle(), &AddNotifyLobbyUpdateReceivedOptions, LobbyUpdateReceivedCallbackObj,
	...
	LobbyMemberUpdateReceivedId = EOS_Lobby_AddNotifyLobbyMemberUpdateReceived(EOSSubsystem->GetLobbyHandle(), &AddNotifyLobbyMemberUpdateReceivedOptions, LobbyMemberUpdateReceivedCallbackObj,
	...
	LobbyMemberStatusReceivedId = EOS_Lobby_AddNotifyLobbyMemberStatusReceived(EOSSubsystem->GetLobbyHandle(), &AddNotifyLobbyMemberStatusReceivedOptions, LobbyMemberStatusReceivedCallbackObj,
	...
	LobbyInviteAcceptedId = EOS_Lobby_AddNotifyLobbyInviteAccepted(EOSSubsystem->GetLobbyHandle(), &AddNotifyLobbyInviteAcceptedOptions, LobbyInviteAcceptedCallbackObj,
	...
	JoinLobbyAcceptedId = EOS_Lobby_AddNotifyJoinLobbyAccepted(EOSSubsystem->GetLobbyHandle(), &AddNotifyJoinLobbyAcceptedOptions, JoinLobbyAcceptedCallbackObj,
...
uint32 FEOSWrapperSessionManager::FindLobbySession(int32 SearchingPlayerNum, const TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
	...
	EOS_EResult SearchResult = EOS_Lobby_CreateLobbySearch(EOSSubsystem->GetLobbyHandle(), &CreateLobbySearchOptions, &LobbySearchHandle);
...
uint32 FEOSWrapperSessionManager::CreateLobbySession(int32 HostingPlayerNum, FNamedOnlineSession* Session)
{
	...
	CreateLobbyOptions.BucketId = EOSSubsystem->BucketIdAnsi;
...
// #endif
	...
	EOS_Lobby_CreateLobby(EOSSubsystem->GetLobbyHandle(), &CreateLobbyOptions, CallbackObj, CallbackObj->GetCallbackPtr());
...
uint32 FEOSWrapperSessionManager::JoinLobbySession(int32 PlayerNum, FNamedOnlineSession* Session, const FOnlineSession* SearchSession)
{
	...
	if (Session->SessionInfo.IsValid())
	{
		...
		if (EOSSessionInfo->SessionId->IsValid())
		{
			...
			EOS_Lobby_JoinLobby(EOSSubsystem->GetLobbyHandle(), &JoinLobbyOptions, CallbackObj, CallbackObj->GetCallbackPtr());
...
uint32 FEOSWrapperSessionManager::DestroyLobbySession(FNamedOnlineSession* Session, const FOnDestroySessionCompleteDelegate& CompletionDelegate)
{
	...
	if (Session->SessionInfo.IsValid())
	{
		...
		EOS_Lobby_LeaveLobby(EOSSubsystem->GetLobbyHandle(), &LeaveOptions, LeaveCallbackObj, LeaveCallbackObj->GetCallbackPtr());
...
bool FEOSWrapperSessionManager::SendLobbyInvite(FName SessionName, EOS_ProductUserId SenderId, EOS_ProductUserId ReceiverId)
{
	...
	EOS_Lobby_SendInvite(EOSSubsystem->GetLobbyHandle(), &SendInviteOptions, CallbackObj, CallbackObj->GetCallbackPtr());
...
void FEOSWrapperSessionManager::OnLobbyUpdateReceived(const EOS_LobbyId& LobbyId)
{
	...
	if (Session)
	{
		...
		EOS_EResult CopyLobbyDetailsResult = EOS_Lobby_CopyLobbyDetailsHandle(EOSSubsystem->GetLobbyHandle(), &Options, &LobbyDetailsHandle);
		if (CopyLobbyDetailsResult == EOS_EResult::EOS_Success)
		{
			...
			{
				UE_LOG_ONLINE(Warning, TEXT("[FEOSWrapperLobby::OnLobbyUpdateReceived] EOS_LobbyDetails_CopyInfo not successful. Finished with EOS_EResult %s"), ANSI_TO_TCHAR(EOS_EResult_ToString(CopyInfoResult)));
			}
		}
		else
		{
			UE_LOG_ONLINE(Warning, TEXT("[FEOSWrapperLobby::OnLobbyUpdateReceived] EOS_Lobby_CopyLobbyDetailsHandle not successful. Finished with EOS_EResult %s"), ANSI_TO_TCHAR(EOS_EResult_ToString(CopyLobbyDetailsResult)));
		}
	}
	else
	{
...
void FEOSWrapperSessionManager::OnLobbyInviteAccepted(const char* InviteId, const EOS_ProductUserId& LocalUserId, const EOS_ProductUserId& TargetUserId)
{
	...
	EOS_EResult Result = EOS_Lobby_CopyLobbyDetailsHandleByInviteId(EOSSubsystem->GetLobbyHandle(), &Options, &LobbyDetailsHandle);
	if (Result == EOS_EResult::EOS_Success)
	{
		TSharedRef<FLobbyDetailsEOS> LobbyDetails = MakeShared<FLobbyDetailsEOS>(LobbyDetailsHandle);
...
void FEOSWrapperSessionManager::OnJoinLobbyAccepted(const EOS_ProductUserId& LocalUserId, const EOS_UI_EventId& UiEventId)
{
	...
	EOS_HLobbyDetails LobbyDetailsHandle;
	EOS_EResult Result = EOS_Lobby_CopyLobbyDetailsHandleByUiEventId(EOSSubsystem->GetLobbyHandle(), &Options, &LobbyDetailsHandle);
	if (Result == EOS_EResult::EOS_Success)
	{
		TSharedRef<FLobbyDetailsEOS> LobbyDetails = MakeShared<FLobbyDetailsEOS>(LobbyDetailsHandle);
...
void FEOSWrapperSessionManager::UpdateOrAddLobbyMember(const FUniqueNetIdEOSLobbyRef& LobbyNetId, const FUniqueNetIdEOSRef& PlayerId)
{
	if (FNamedOnlineSession* Session = GetNamedSessionFromLobbyId(*LobbyNetId))
	{
		...
		if (FSessionSettings* MemberSettings = Session->SessionSettings.MemberSettings.Find(PlayerId))
		{
			...
			EOS_EResult Result = EOS_Lobby_CopyLobbyDetailsHandle(EOSSubsystem->GetLobbyHandle(), &Options, &LobbyDetailsHandle);
			if (Result == EOS_EResult::EOS_Success)
			{
